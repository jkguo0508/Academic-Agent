...
async def search_node(state: State) -> State:
    ...
    try:
        ...
        # 请求级隔离：优先使用本任务的 user_proxy，找不到时回退到全局实例
        task_id = state.get("task_id") or getattr(current_state, "task_id", None)
        ctx = task_registry.get(task_id)
        active_user_proxy = ctx.user_proxy if ctx else userProxyAgent

        result = await active_user_proxy.on_messages(
