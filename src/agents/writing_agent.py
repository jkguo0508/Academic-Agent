...
async def writing_node(state: State) -> State:
    ...
    try:
        ...
        # 写作层基于结构化分析对象 AnalysisResults，而非 JSON 字符串
        analysis = current_state.analyse_results
        writing_state["analysis"] = analysis
        writing_state["global_analysis"] = analysis.to_prompt_text() if analysis else ""
